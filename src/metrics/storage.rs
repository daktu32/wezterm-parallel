...
impl MetricsStorage {
    ...
    pub async fn save_process_metrics(&self, metrics: &[ProcessMetrics]) -> Result<(), String> {
        ...
        for metric in metrics {
            by_process.entry(metric.process_id.clone())
        ...
        // Add to cache
        {
            ...
            for (process_id, process_metrics) in by_process {
                let cached = cache.process_metrics.entry(process_id).or_insert_with(Vec::new);
    ...
    pub async fn save_workspace_metrics(&self, metrics: &WorkspaceMetrics) -> Result<(), String> {
        ...
        {
            let mut cache = self.cache.write().await;
            let cached = cache.workspace_metrics
    ...
    pub async fn get_recent_process_metrics(&self, process_id: &str, limit: usize) -> Vec<ProcessMetrics> {
        let cache = self.cache.read().await;
    ...
    pub async fn get_recent_workspace_metrics(&self, workspace_name: &str, limit: usize) -> Vec<WorkspaceMetrics> {
        let cache = self.cache.read().await;
    ...
    pub async fn load_metrics_range(&self, metric_type: &str, start_time: u64, end_time: u64) -> Result<Vec<serde_json::Value>, String> {
            ...
            if file_path.exists() {
                match self.load_file(&file_path).await {
                    Ok(metrics) => {
                        ...
                        let filtered: Vec<serde_json::Value> = metrics.into_iter()
                            .filter(|m| {
    ...
    pub async fn cleanup_old_files(&self, retention_hours: u64) -> Result<(), String> {
        info!("Cleaning up metrics files older than {} hours", retention_hours);
            ...
            if type_dir.exists() {
                ...
                while let Some(entry) = entries.next_entry().await
                    ...
                    if let Some(file_name) = path.file_name().and_then(|n| n.to_str()) {
                        ...
                        if let Some(timestamp_str) = file_name.strip_prefix("metrics_")
                            .and_then(|s| s.strip_suffix(".json")) {
                            
                            if let Ok(timestamp) = timestamp_str.parse::<u64>() {
                                if timestamp < cutoff_time {
                                    match fs::remove_file(&path).await {
                                        Ok(_) => debug!("Removed old metrics file: {}", path.display()),
    ...
    fn get_metrics_file_path(&self, metric_type: &str, timestamp: u64) -> PathBuf {
        ...
        self.base_path
            .join(metric_type)
            .join(format!("metrics_{}.json", hour_timestamp))
    }
    
    /// Append metrics to file
    async fn append_to_file<T: serde::Serialize>(&self, path: &Path, data: &T) -> Result<(), String> {
    ...
    async fn append_batch_to_file<T: serde::Serialize>(&self, path: &Path, data: &[T]) -> Result<(), String> {
        // Create parent directory if needed
        if let Some(parent) = path.parent() {
            fs::create_dir_all(parent).await
                .map_err(|e| format!("Failed to create directory: {}", e))?;
        ...
        file.write_all(json_lines.as_bytes()).await
            .map_err(|e| format!("Failed to write to file: {}", e))?;
    ...
    async fn rotate_file(&self, path: &Path) -> Result<(), String> {
        ...
        info!("Rotated large metrics file: {} -> {}", path.display(), new_path.display());
    ...
    async fn load_recent_metrics(&self) -> Result<(), String> {
        ...
        for metric_type in &["system", "process", "workspace", "framework"] {
            match self.load_metrics_range(metric_type, one_hour_ago, current_time).await {
                Ok(metrics) => {
                    debug!("Loaded {} {} metrics", metrics.len(), metric_type);
                    // Could deserialize and add to cache here if needed
                }
                Err(e) => warn!("Failed to load {} metrics: {}", metric_type, e),
            }
        }
    ...
    pub async fn get_storage_stats(&self) -> Result<StorageStats, String> {
        ...
        for metric_type in &["system", "process", "workspace", "framework"] {
            ...
            metrics_by_type.insert(metric_type.to_string(), TypeStats {
    ...
    async fn estimate_cache_size(&self) -> usize {
        ...
        let process_size = cache.process_metrics.values()
            .map(|v| v.len() * std::mem::size_of::<ProcessMetrics>())
            .sum::<usize>();
        let workspace_size = cache.workspace_metrics.values()
            .map(|v| v.len() * std::mem::size_of::<WorkspaceMetrics>())
            .sum::<usize>();
        let framework_size = cache.framework_metrics.len() * std::mem::size_of::<FrameworkMetrics>();
...
mod tests {
    ...
    async fn test_metrics_storage() {
        ...
        let storage = MetricsStorage::new(temp_dir.path().to_path_buf()).await.unwrap();
