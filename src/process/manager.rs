    ...
    pub async fn spawn_process(
        ...
        // Check process limit
        {
            ...
            if processes.len() >= self.config.max_processes {
                return Err(format!("Maximum process limit ({}) reached", self.config.max_processes));
            }
        }
        ...
        let process_info = ProcessInfo {
            id: process_id.clone(),
            command: format!("{} {}", self.config.claude_code_binary, command_args.join(" ")),
        ...
        self.update_process_status(&process_id, ProcessStatus::Running).await;
    ...
    pub async fn restart_process(&self, process_id: &str) -> Result<(), String> {
        ...
        let (workspace, command_args, restart_count) = {
            ...
            if let Some(managed_process) = processes.get(process_id) {
                let command_parts: Vec<String> = managed_process.info.command
                    .split_whitespace()
        ...
        // Spawn new process
        self.spawn_process(process_id.to_string(), workspace, command_args).await?;
...
mod tests {
    ...
    async fn test_spawn_process() {
        ...
        if let Some(event) = receiver.recv().await {
            match event {
                ProcessEvent::Started { process_id, workspace, .. } => {
    ...
    async fn test_duplicate_process_id() {
        ...
        let result1 = manager.spawn_process(
            "test-process".to_string(),
            "test-workspace".to_string(),
            vec!["hello".to_string()],
        ).await;
        ...
        let result2 = manager.spawn_process(
            "test-process".to_string(),
            "test-workspace".to_string(),
            vec!["hello".to_string()],
        ).await;
