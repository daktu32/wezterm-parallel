...
mod tests {
    ...
    async fn test_update_health_history_adds_entries() {
        ...
        // 履歴を更新
        health_manager.update_health_history(health_check.clone()).await;
    ...
    fn test_health_status_to_string_conversion() {
        ...
        assert_eq!(health_status_to_string(&HealthStatus::Unhealthy), "UNHEALTHY");
    ...
    async fn test_failure_count_reset_on_recovery() {
        ...
        // 失敗
        let component_health1 = health_manager.create_component_health(&unhealthy_result).await;
        assert_eq!(component_health1.failure_count, 1);
        
        // 回復
        let component_health2 = health_manager.create_component_health(&healthy_result).await;
