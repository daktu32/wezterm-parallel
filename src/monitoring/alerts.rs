...
impl AlertManager {
    ...
    pub fn add_notification_sender(&mut self, sender: Box<dyn AlertNotificationSender + Send + Sync>) {
        info!("Added alert notification sender: {}", sender.name());
        self.notification_senders.push(sender);
    }
    
    /// Start alert processing
    pub async fn start(&self, mut metrics_rx: mpsc::Receiver<SystemMetrics>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    async fn evaluate_metrics(&self, metrics: &SystemMetrics) -> Result<(), Box<dyn std::error::Error>> {
        debug!("Evaluating metrics for alerts at timestamp: {}", metrics.timestamp);
    ...
    async fn update_evaluation_state(&self, metrics: &SystemMetrics) {
        ...
        if state.cpu_history.len() > 60 { // Keep last 60 samples
    ...
    /// Check system-level alerts
    async fn check_system_alerts(&self, metrics: &SystemMetrics) -> Result<(), Box<dyn std::error::Error>> {
        // CPU usage alert
        if metrics.cpu_usage > self.thresholds.cpu_usage {
            self.create_alert_if_needed(
                ...
                "System",
                format!("High CPU usage: {:.2}% (threshold: {:.2}%)", 
        ...
        let memory_usage_percentage = if metrics.memory_usage + metrics.memory_available > 0 {
            (metrics.memory_usage as f64 / (metrics.memory_usage + metrics.memory_available) as f64) * 100.0
        } else {
            0.0
        };
        ...
        if memory_usage_percentage > self.thresholds.memory_usage {
            self.create_alert_if_needed(
                ...
                AlertSeverity::Warning,
                "System",
                format!("High memory usage: {:.2}% (threshold: {:.2}%)", 
        ...
        let disk_usage_percentage = if metrics.disk_usage + metrics.disk_available > 0 {
            (metrics.disk_usage as f64 / (metrics.disk_usage + metrics.disk_available) as f64) * 100.0
        } else {
            0.0
        };
        ...
        if disk_usage_percentage > self.thresholds.disk_usage {
            self.create_alert_if_needed(
                ...
                "System",
                format!("High disk usage: {:.2}% (threshold: {:.2}%)", 
    ...
    /// Check process-level alerts
    async fn check_process_alerts(&self, metrics: &SystemMetrics) -> Result<(), Box<dyn std::error::Error>> {
        for (process_name, process_metrics) in &metrics.process_metrics {
            ...
            if process_metrics.restart_count > self.thresholds.restart_count {
                self.create_alert_if_needed(
                    ...
                    AlertSeverity::Error,
                    "Process",
                    format!("Process {} has restarted {} times (threshold: {})", 
            ...
            if process_metrics.status == super::ProcessStatus::Failed {
                self.create_alert_if_needed(
                    ...
                    AlertSeverity::Critical,
                    "Process",
                    format!("Process {} has failed", process_name),
                    Some(process_name),
                    metrics.timestamp,
                    serde_json::json!({
                        "process_name": process_name,
            ...
            if process_metrics.cpu_usage > 80.0 { // Process-specific threshold
                self.create_alert_if_needed(
                    &format!("process_cpu_high_{}", process_name),
                    AlertSeverity::Warning,
                    "Process",
                    format!("Process {} high CPU usage: {:.2}%", process_name, process_metrics.cpu_usage),
                    Some(process_name),
                    metrics.timestamp,
                    serde_json::json!({
                        "process_name": process_name,
    ...
    /// Check for resolved alerts
    async fn check_resolved_alerts(&self, metrics: &SystemMetrics) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for (alert_id, alert) in active_alerts.iter() {
            ...
            match alert.category.as_str() {
                "System" => {
                    if alert_id == "system_cpu_high" && metrics.cpu_usage <= self.thresholds.cpu_usage * 0.9 {
                        should_resolve = true;
                ...
                "Process" => {
                    ...
                    for (process_name, process_metrics) in &metrics.process_metrics {
                        if alert_id.contains(process_name) {
                            if alert_id.contains("failed") && process_metrics.status == super::ProcessStatus::Running {
                                should_resolve = true;
                            } else if alert_id.contains("cpu_high") && process_metrics.cpu_usage <= 70.0 {
                                should_resolve = true;
                            }
                        }
                    }
    ...
    async fn create_alert_if_needed(
        ...
        if let Some(&last_time) = state.last_alert_times.get(alert_id) {
            if timestamp - last_time < 300 { // 5 minutes cooldown
                return Ok(());
            }
        }
        ...
        state.last_alert_times.insert(alert_id.to_string(), timestamp);
        drop(state);
    ...
    async fn send_alert_notification(&self, alert: &Alert) {
        for sender in &self.notification_senders {
            if let Err(e) = sender.send_alert_sync(alert) {
                error!("Failed to send alert notification via {}: {}", sender.name(), e);
            }
        }
    }
...
impl AlertNotificationSender for ConsoleAlertSender {
    fn send_alert_sync(&self, alert: &Alert) -> Result<(), Box<dyn std::error::Error>> {
        ...
        println!("{} [{}] {} - {}", severity_icon, status, alert.category, alert.message);
...
impl AlertNotificationSender for LogAlertSender {
    fn send_alert_sync(&self, alert: &Alert) -> Result<(), Box<dyn std::error::Error>> {
        ...
        std::fs::write(&self.log_path, format!("{}\n", alert_json))?;
        Ok(())
    }
...
impl AlertNotificationSender for WebhookAlertSender {
    fn send_alert_sync(&self, alert: &Alert) -> Result<(), Box<dyn std::error::Error>> {
        ...
        tracing::info!("Would send webhook alert to {}: {}", self.webhook_url, alert.message);
        Ok(())
    }
...
mod tests {
    ...
    async fn test_alert_spam_prevention() {
        ...
        // Create first alert
        manager.create_alert_if_needed(
            "test_alert",
            AlertSeverity::Warning,
            "Test",
