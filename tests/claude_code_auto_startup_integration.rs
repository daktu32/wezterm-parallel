...
use wezterm_parallel::process::{
    ClaudeCodeDetector, ClaudeCodeConfigBuilder,
    ClaudeHealthMonitor, HealthConfig,
...
async fn test_claude_code_config_builder() {
    ...
    assert!(config.get_complete_arguments().contains(&"--test".to_string()));
...
async fn test_workspace_auto_startup_integration() {
    ...
    // ワークスペース作成（自動起動機能付き）
    let result = env.workspace_manager.create_workspace("test-auto-startup", "basic").await;
    ...
    if result.is_ok() {
        ...
        // ワークスペース情報を確認
        if let Some(workspace_info) = env.workspace_manager.get_workspace_info("test-auto-startup").await {
            println!("Created workspace: {:?}", workspace_info.name);
            // Note: 実際のClaude Codeプロセスは起動しないが、ワークスペース構造は確認できる
        }
    } else {
        println!("Workspace creation failed (expected in test environment): {:?}", result.err());
    }
}
...
async fn test_health_monitoring_functionality() {
    ...
    // 監視を開始
    let result = env.health_monitor.start_monitoring(
...
async fn test_full_integration_workflow() {
    ...
    match env.workspace_manager.create_workspace(workspace_name, "basic").await {
        Ok(_) => {
            ...
            // 4. ワークスペース情報を取得
            if let Some(workspace_info) = env.workspace_manager.get_workspace_info(workspace_name).await {
                println!("Workspace info: name={}, processes={}", 
                    workspace_info.name, 
    ...
    // 5. ヘルスモニタリングをテスト
    let monitor_result = env.health_monitor.start_monitoring(
    ...
    if monitor_result.is_ok() {
        ...
        // 7. 監視を停止
        let _ = env.health_monitor.stop_monitoring("integration-test-process").await;
    }
...
async fn test_error_handling_and_edge_cases() {
    ...
    // 1. 存在しないワークスペースの操作
    let result = env.workspace_manager.get_workspace_info("nonexistent-workspace").await;
    ...
    // 3. 存在しないプロセスの監視停止
    let result = env.health_monitor.stop_monitoring("nonexistent-process").await;
...
async fn test_performance_and_scalability() {
    ...
    for i in 0..workspace_count {
        ...
        match env.workspace_manager.create_workspace(&workspace_name, "basic").await {
            Ok(_) => {
                ...
                {
                    let logger = env.logger.lock().await;
                    let _ = logger.start_logging_process(
                        format!("process-{}", i),
                ...
                // 各ワークスペースでヘルス監視を開始
                let _ = env.health_monitor.start_monitoring(
                    format!("process-{}", i),
                    workspace_name,
                    Some(1000 + i as u32)
                ).await;
    ...
    // クリーンアップ
    for i in 0..workspace_count {
        let _ = env.health_monitor.stop_monitoring(&format!("process-{}", i)).await;
        {
            let logger = env.logger.lock().await;
