...
fn test_task_dependency_detection() {
    ...
    let mut task1 = DistributedTask::new(
        "Task 1".to_string(),
        TaskPriority::High,
        vec![],
    );
...
fn test_optimal_process_assignment() {
    ...
    distributor.update_process_load(process1_id, ProcessLoad {
        cpu_usage: 0.8,
        memory_usage: 0.6,
        active_tasks: 3,
    });
    
    distributor.update_process_load(process2_id, ProcessLoad {
        cpu_usage: 0.2,
        memory_usage: 0.3,
        active_tasks: 1,
    });
    
    distributor.update_process_load(process3_id, ProcessLoad {
        cpu_usage: 0.5,
        memory_usage: 0.4,
        active_tasks: 2,
    });
...
fn test_dependency_graph_resolution() {
    ...
    // BとCは順不同だが、両方ともDより前
    let b_index = execution_order.iter().position(|&id| id == task_b_id).unwrap();
    let c_index = execution_order.iter().position(|&id| id == task_c_id).unwrap();
    let d_index = execution_order.iter().position(|&id| id == task_d_id).unwrap();
...
fn test_resource_based_assignment() {
    ...
    distributor.update_process_load(cpu_process_id, ProcessLoad {
        cpu_usage: 0.1, // CPU余裕あり
        memory_usage: 0.7, // メモリ不足気味
        active_tasks: 1,
    });
    
    distributor.update_process_load(memory_process_id, ProcessLoad {
